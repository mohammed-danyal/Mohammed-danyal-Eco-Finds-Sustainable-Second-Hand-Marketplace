...
# 🔧 Page Configuration
# ----------------------------
st.set_page_config(page_title="Second-Hand Marketplace", layout="wide")
...
def get_resized_image_bytes(url: str, max_size=(100, 100)):
    ...
    try:
        ...
        # don't st.warning() inside cached func (it can cause issues); return None and warn outside
        return None
...
listings = [
    {
        ...
        # replaced with a stable sample image URL
...
for idx, item in enumerate(filtered_listings):
    with cols[idx % 3]:
        # Product Image: try to load resized bytes, fall back to URL if resize fails
        img_bytes = get_resized_image_bytes(item["image"], max_size=(400, 300))
        if img_bytes:
            st.image(img_bytes, use_container_width=True)
        else:
            # fallback to displaying the URL directly (Streamlit will attempt to fetch)
            try:
