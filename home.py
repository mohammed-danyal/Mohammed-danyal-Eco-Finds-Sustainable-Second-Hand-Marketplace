...
def get_resized_image_bytes(url: str, max_size=(400, 300)):
    try:
...
def homepage():
    ...
    st.write("Browse the latest listings 👇")
    ...
    # Show products in a responsive grid (3 per row)
...
def product_detail(product):
    ...
    st.write(f"📍 {product['location']} | 📦 {product['category']}")

    img_bytes = get_resized_image_bytes(product["image_url"], max_size=(600, 400))
    if img_bytes:
        st.image(img_bytes, use_column_width="always")
    ...
    st.write(f"**{product['seller_name']}** (Since {product['seller_since']})")
    st.write(f"📞 {product['seller_phone']}")
    st.write(f"📩 {product['seller_email']}")

    if product["seller_photo"]:
        seller_img = get_resized_image_bytes(product["seller_photo"], max_size=(150, 150))
        if seller_img:
            st.image(seller_img, caption="Seller", width=120)
...
if st.session_state.page == "Home":
    homepage()
elif st.session_state.page == "Detail" and st.session_state.selected_product:
    product_detail(st.session_state.selected_product)
"""
import streamlit as st
from io import BytesIO
from PIL import Image
